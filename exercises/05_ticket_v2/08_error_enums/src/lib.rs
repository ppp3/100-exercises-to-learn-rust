...
#[derive(Debug)]
enum TicketNewError {TitleError(String), DescriptionError(String)}
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    {
        Ok(ticket) => ticket,
        Err(TitleError(str)) => {panic!("{}",str)}
        Err(DescriptionError(_)) => Ticket::new(title, "Description not provided".to_string(), status).unwrap()
    }
}
